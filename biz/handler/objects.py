...
class Producer(object):
    def __init__(self, amqp_url, exchange, routing_key, exchange_type):
        self._amqp_url = amqp_url
        self._exchange = exchange
        self._routing_key = routing_key
        self._exchange_type = exchange_type
    def send(self, *args, **kwargs):
        """The arguments are always converted to yaml, which can also in most cases be read by json.
        The messages can therefore be consumed by any programming language."""
        msg = yaml.safe_dump((args, kwargs))
        connection = pika.BlockingConnection(pika.URLParameters(self._amqp_url))
        channel = connection.channel()
        channel.exchange_declare(self._exchange, type=self._exchange_type)
        channel.basic_publish(exchange=self._exchange, routing_key=self._routing_key, body=msg)
        channel.close()
...
class Consumer(Thread):
    ...
    def __init__(self, amqp_url, exchange, routing_key, queue=None, exchange_type='direct'):
        ...
        self.queue = queue or 'q.' + routing_key
    ...
    def on_message(self, *args, **kwargs):
        """Transparently receive all arguments from the message as values in the args and kwargs.
        Must be overridden by using application."""
        raise NotImplementedError

    def on_raw_message(self, unused_channel, basic_deliver, properties, body):
        ...
        """
        try:
            args, kwargs = yaml.safe_load(body)
            self.on_message(*args, **kwargs)
            self.acknowledge_message(basic_deliver.delivery_tag)
        except Exception as e:
            LOGGER.error('Delivery failed: {}'.format(str(e)))
    ...
    def start_consuming(self):
        ...
        self._consumer_tag = self._channel.basic_consume(self.on_raw_message,
                                                         self.queue)
